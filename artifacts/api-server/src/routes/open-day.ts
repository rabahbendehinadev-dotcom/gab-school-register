...
router.get("/open-day/status", async (_req, res): Promise<void> => {
  const [enabled, seats, date, opensAt, title, sectionVisible] = await Promise.all([
    getSetting(KEYS.enabled), getSetting(KEYS.seats), getSetting(KEYS.date),
    getSetting(KEYS.opensAt), getSetting(KEYS.title), getSetting(KEYS.sectionVisible),
  ]);
  const [countResult] = await db.select({ count: sql<number>`count(*)::int` }).from(openDayRegistrationsTable);
  ...
  res.json({
    ...
    seats: totalSeats, date: date ?? null, opensAt: opensAt ?? null,
    title: title ?? "اليوم المفتوح",
    registrationCount, spotsLeft: Math.max(0, totalSeats - registrationCount),
    isFull: registrationCount >= totalSeats,
  });
});

router.get("/open-day/registrations", requirePermission("manage_notifications"), async (_req, res): Promise<void> => {
  const registrations = await db.select().from(openDayRegistrationsTable).orderBy(openDayRegistrationsTable.createdAt);
...
router.post("/open-day/register", async (req, res): Promise<void> => {
  ...
  if (!firstName || !lastName || !phone || !city) {
    res.status(400).json({ error: "الرجاء ملء جميع الحقول المطلوبة" }); return;
  }
  const [enabled, seats] = await Promise.all([getSetting(KEYS.enabled), getSetting(KEYS.seats)]);
  if (enabled !== "true") { res.status(403).json({ error: "التسجيل في اليوم المفتوح غير متاح حالياً" }); return; }
  const [countResult] = await db.select({ count: sql<number>`count(*)::int` }).from(openDayRegistrationsTable);
  const totalSeats = Number(seats ?? "15");
  const registrationCount = countResult?.count ?? 0;
  if (registrationCount >= totalSeats) { res.status(409).json({ error: "عذراً، لقد امتلأت جميع المقاعد" }); return; }
  const [reg] = await db.insert(openDayRegistrationsTable).values({ firstName, lastName, phone, whatsapp: whatsapp || phone, city }).returning();
  ...
  const waText = encodeURIComponent(`مرحباً ${firstName}، تم تأكيد تسجيلك في اليوم المفتوح لـ GAB SCHOOL! 🎉`);
  const waLink = `https://wa.me/${waNumber}?text=${waText}`;
  const spotsLeft = Math.max(0, totalSeats - registrationCount - 1);
  await sendTelegramNotification([
    `🎟️ <b>تسجيل جديد في اليوم المفتوح!</b>`, ``,
    `👤 <b>الاسم:</b> ${firstName} ${lastName}`, `📞 <b>الهاتف:</b> ${phone}`,
    `💬 <b>الواتساب:</b> ${whatsapp || phone}`, `📍 <b>الولاية:</b> ${city}`,
...
router.put("/open-day/settings", requirePermission("manage_notifications"), async (req, res): Promise<void> => {
  ...
  if (enabled !== undefined) {
    ...
    if (sectionVisible === undefined) ops.push(setSetting(KEYS.sectionVisible, enabled ? "true" : "false"));
  }
