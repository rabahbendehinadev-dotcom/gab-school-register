...
router.get("/students/:id/payments", requireRole("admin", "manager", "staff", "assistant"), async (req, res): Promise<void> => {
...
// Record a payment
router.post("/students/:id/payments", requireRole("admin", "manager", "staff"), async (req, res): Promise<void> => {
  const id = parseInt(String(req.params.id), 10);
  if (Number.isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
  ...
  await logActivity("payment_recorded", `💰 دفعة ${parsed.data.amount} دج (${parsed.data.type}) — ${student.firstName} ${student.lastName}`, performer, id);

  res.status(201).json(payment);
});

// Delete a payment (admin/manager only)
router.delete("/payments/:id", requireRole("admin", "manager"), async (req, res): Promise<void> => {
  const id = parseInt(String(req.params.id), 10);
  if (Number.isNaN(id)) { res.status(400).json({ error: "Invalid id" }); return; }
...
router.get("/stats/financials", requireRole("admin", "manager"), async (_req, res): Promise<void> => {
  const now = new Date();
  ...
  // Outstanding: sum of (agreedPrice - paid) for non-archived students who have an agreed price
  ...
  res.json({
    ...
    totalRevenue: Number(allRow.total),
    outstanding,
  });
});
