...
const REALERT_MS = 10 * 60 * 1000; // re-alert every 10 minutes while unhandled exist
...
export function NotificationCenter() {
  ...
  // Detect new notifications + drive the re-alert loop
  useEffect(() => {
    const topUnhandled = unhandled[0] ?? null;

    // Seed on first load so we don't blast a chime for pre-existing items
    ...
    // A genuinely new unhandled notification arrived
  ...
  // Persistent re-alert: while unhandled items remain, re-chime every 10 minutes
  useEffect(() => {
  ...
  const labels = {
    ...
    allHandled: lang === "fr" ? "Tout est traité ✅" : "تم التعامل مع الكل ✅",
  };

  return (
    <div className="relative" ref={panelRef} dir={lang === "fr" ? "ltr" : "rtl"}>
      <button
      ...
      {/* Floating popup for newly-arrived notifications */}
      {popup && (
        <div className={`fixed bottom-6 z-[60] w-[340px] max-w-[calc(100vw-2rem)] rounded-2xl border-2 border-primary/30 bg-card shadow-2xl p-4 animate-fadeup ${lang === "fr" ? "right-6" : "left-6"}`} dir={lang === "fr" ? "ltr" : "rtl"}>
          <button onClick={() => setPopup(null)} className="absolute top-2 left-2 p-1 rounded-full hover:bg-muted">
