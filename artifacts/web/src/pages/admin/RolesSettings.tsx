        ...
        {PERMISSION_GROUPS.map(group => {
          ...
          return (
            <div key={group.label} className="border border-border/40 rounded-xl overflow-hidden">
              <button
                type="button"
                className="w-full flex items-center justify-between px-3 py-2 bg-muted/30 hover:bg-muted/60 transition-colors text-sm font-medium"
                  ...
                  {group.perms.map(perm => (
                    <label key={perm} className={`flex items-center gap-2 px-2 py-1.5 rounded-lg text-xs cursor-pointer transition-colors ${perms.includes(perm) ? "bg-primary/10 text-primary" : "text-muted-foreground hover:bg-muted/50"} ${readOnly ? "cursor-default" : ""}`}>
                      <input
                        ...
                        disabled={readOnly}
                        onChange={() => setPerms && !readOnly && togglePerm(perm, perms, setPerms)}
  ...
  return (
    <AdminLayout>
      <div dir="rtl" className="space-y-6">
        ...
        {error && <div className="text-destructive text-sm">ليس لديك صلاحية مشاهدة الأدوار</div>}
          ...
          {roles?.map(role => {
            ...
            return (
              <div key={role.id} className="bg-card border border-border/50 rounded-2xl p-5 shadow-sm">
                    ...
                    <div>
                      {isEditing ? (
                        <input
                          ...
                          className="border border-border rounded-lg px-2 py-1 text-sm font-semibold bg-background"
                        />
                      ) : (
                      ...
                      {isEditing ? (
                        <>
                          ...
                          <Button
                            ...
                            onClick={() => updateMutation.mutate({ id: role.id, displayName: editDisplayName, permissions: editPerms })}
                          >
                        ...
                        <>
                          ...
                          {!role.isSystem && (
                            <Button
                              ...
                              variant="ghost"
                              className="text-destructive hover:text-destructive"
                              onClick={() => { if (confirm("هل أنت متأكد من حذف هذا الدور؟")) deleteMutation.mutate(role.id); }}
                            >
                ...
                {/* Permissions badge summary (collapsed) */}
                {!isEditing && (
                  <div className="mt-3 flex flex-wrap gap-1.5">
                    {role.permissions.slice(0, 8).map(p => (
                      <span key={p} className="text-[10px] bg-primary/8 text-primary px-2 py-0.5 rounded-full font-medium">
                        {PERM_LABELS[p] ?? p}
                      </span>
                    ))}
                    {role.permissions.length > 8 && (
                      <span className="text-[10px] text-muted-foreground px-2 py-0.5">+{role.permissions.length - 8} أخرى</span>
                    )}
                ...
                {/* Edit matrix */}
                {isEditing && (
                  <PermMatrix perms={editPerms} setPerms={setEditPerms} />
                )}
              </div>
            );
          })}
